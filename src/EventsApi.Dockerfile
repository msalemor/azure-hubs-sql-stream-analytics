...
WORKDIR /app
COPY . .
WORKDIR /app/eventsapi

RUN go mod tidy
RUN CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build -a -v -installsuffix cgo -ldflags '-extldflags "-static"' -tags timetzdata -o eventsapi .
...
# Deploy image
FROM scratch
COPY --from=builder /app/eventsapi/eventsapi .
# copy ca certs
